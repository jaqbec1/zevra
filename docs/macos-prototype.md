...
Base import reads the global filters and the selected view. Both must match. A Base with one uniquely named view uses it directly; for multiple views the app asks the user to choose, preselecting `All` when present. The core API retains `All` as its default when available, accepts a sole view under any name, and requires an explicit view name for other multi-view Bases. The supported subset is `categories.contains(link("Name"))`, either as a filter string or in nonempty, nested `and` groups. Multiple required categories are supported. Other expressions (including status, `or`, negation, formulas and `this`), duplicate or empty view names, duplicate filter keys, row limits, grouping and unknown selection properties stop the entire import with an explanation. Filters in unselected views do not apply to the chosen view. Presentation fields such as column order, sort and summaries do not affect membership; import order is file path order.
...
Accepted in the planning discussion: ask the user to point to a source; keep optional Base filtering independent of its location; apply every global/selected-view condition or stop with a clear message. Unsupported forms can be added after testing concrete examples. Test the picker, compare the imported titles with the chosen view, move the Base and repeat, then add an unsupported condition and verify that the profile is unchanged. A local parser trial on user-selected source data passed after removing the view-name restriction. Native picker interaction and live TypeSafe behavior remain separate checks.
...
### Named Base views — 2026-09-25

A local read-only trial found that valid Bases with views named differently from `All` were rejected. The importer now supports arbitrary unique view names and the app offers a view chooser when there is more than one. Global conditions and all selected-view conditions still apply; unsupported status or empty-property expressions still reject the import. No original Base or source file was changed.

`sh macos/check.sh` passed formatting, 40 native tests and the Apple Silicon Debug build. New synthetic cases verify single named views, explicit multi-view selection, combined global/view categories, unsupported selected filters, missing views and duplicate names. A separate local parser trial used the supplied Bases against the user's chosen notes source; all four selected category views returned candidates. No private titles, links, counts or source files are included in the repository. The new native view chooser has compiled but has not yet been exercised through UI automation. No live provider call, persistent import, merge or release was performed.

### Reviewed source import and optional OpenAI analysis

The accepted flow is implemented locally: **Choose source… → review materials → optionally Analyze with OpenAI → review both result groups → Save approved results**. Folder and export selection, including a folder filtered by a Base, now opens an editable draft instead of saving immediately. Cancel discards it. The user can edit titles, links and topics and deselect materials. Supplied links are retained from Markdown frontmatter (`url`, `source`, `link`), JSON/JavaScript title records (`titleUrl`, `url`, `expanded_url`, `link`), HTML anchors and CSV URL/link columns. Unknown or unavailable links remain empty. Separate `website` and `github` properties are not yet mapped to the material link. Existing archive item IDs and ratings survive the optional URL/topic fields; approved edits update a matching item without clearing its rating.
