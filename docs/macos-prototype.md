...
An Obsidian `.base` file can also be selected. Zevra reads its single category filter and imports titles only from notes whose frontmatter matches that category; it does not treat `Consumed` as a positive rating. The current `Consumables.base` uses the `Clippings` category. The importer requires an `All` view and one category filter, so complex Bases are rejected rather than approximated.
