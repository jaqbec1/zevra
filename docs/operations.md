...
Collection must pass both extension and collector policies. Keep the mode and exclusions synchronized. In `allow` mode, a missing allowed domain on either side means no stored page; `exclude` mode accepts other sites after applying exclusions. The collector config is read at startup. Use the extension's options page to inspect delivery errors. A healthy `/health` response does not prove the token or event delivery works.
